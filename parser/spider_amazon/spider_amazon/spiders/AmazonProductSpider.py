...
class AmazonproductspiderSpider(scrapy.Spider):
    ...
    allowed_domains = ['mvideo.ru', 'scraperapi.com']
    ...
    start_urls = ['https://www.mvideo.ru/?ysclid=m9r53v8rye608991778']
    ...
    def start_requests(self):
        # URL к внутреннему API, который возвращает JSON с товарами
        # Например, так можно получить смартфоны на первой странице
        api_url = 'https://www.mvideo.ru/bff/products/listing?categoryId=smartfony-205&offset=0&limit=24&filterParams=mount%3Dtrue'
    ...
    def parse_product_json(self, response):
        ...
        # Список productIds, который нужен для получения полной информации
        product_ids = data.get("body", {}).get("products", [])

        if not product_ids:
            self.logger.warning("Нет продуктов в ответе.")
            return

        ids_string = ','.join(product_ids)

        # Запрос на получение полной информации по productIds
        details_url = f'https://www.mvideo.ru/bff/product-details?productIds={ids_string}'

        yield scrapy.Request(
            url=get_url(details_url),
    ...
    def parse_product_details(self, response):
        ...
        for product in products:
            url = f"https://www.mvideo.ru/products/{product['nameTranslit']}-{product['productId']}"
            yield {
    ...
    # base_url = "https://www.mvideo.ru"  # Убедитесь, что это правильный домен
        product_cards = response.css('mvid-product-card-grid')
        for card in product_cards:
            product_link = card.css('mvid-plp-product-title a::attr(href)').get()
    ...
    def parse_phone(self, response):
        ...
        price_text = response.css('span.price__main-value ::text').get()
        price_number = ''.join(filter(str.isdigit, price_text))  # -> "15999"
